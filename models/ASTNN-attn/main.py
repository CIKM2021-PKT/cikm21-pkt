...
def main(fold):
    ...
    if parser.parse_args().dataset == 'codeforces':
        ...
        ast_embeddings = np.zeros((word2vec.syn0.shape[0] + 1, word2vec.syn0.shape[1]), dtype="float32")
        ast_embeddings[:word2vec.syn0.shape[0]] = word2vec.syn0

        parser.add_argument('--max_tokens', type=int, default=word2vec.syn0.shape[0] + 1)
        parser.add_argument('--ast_embed_dim', type=int, default=word2vec.syn0.shape[1])
        parser.add_argument('--ast_encode_dim', type=int, default=128)
